    ...
    def __init__(self, growth_rate=12, block_config=(16, 16, 16, 16), compression=0.5, init_channel=1,
        ...
        if small_inputs:
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv2d(init_channel, num_init_features, kernel_size=3, stride=1, padding=1, bias=False)),
            ]))
        else:
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv2d(init_channel, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
            ]))
